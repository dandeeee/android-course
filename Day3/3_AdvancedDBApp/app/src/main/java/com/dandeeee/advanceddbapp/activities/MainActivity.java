...
public class MainActivity extends Activity implements OnItemClickListener {
	...
	private void fetchDataFromDB() {
		...
		Cursor cursor = db.query(DBSchema.PERSONS, columns, null, null, null, null, null);
		...
		if (!cursor.isAfterLast()) {
			...
			while(!cursor.isAfterLast()) {
				...
				Person p = new Person();
				p.personId = cursor.getInt(cursor.getColumnIndex(DBSchema.PERSONID));
				p.name = cursor.getString(cursor.getColumnIndex(DBSchema.NAME));
				p.address = cursor.getString(cursor.getColumnIndex(DBSchema.ADDRESS));
				p.phone = cursor.getString(cursor.getColumnIndex(DBSchema.PHONE));
				p.email = cursor.getString(cursor.getColumnIndex(DBSchema.EMAIL));
				p.date = cursor.getString(cursor.getColumnIndex(DBSchema.DATE));
				p.time = cursor.getString(cursor.getColumnIndex(DBSchema.TIME));
				p.rating = cursor.getFloat(cursor.getColumnIndex(DBSchema.RATING));
		...
		if (item.getTitle().equals("Delete")) {
			...
			builder.setPositiveButton("Yes", new OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					String whereClause = String.format("%s=%d", DBSchema.PERSONID, p.personId);
